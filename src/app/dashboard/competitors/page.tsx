...
import { getBusinessByOwner, getCompetitorsByBusiness, getCompetitorHistory, Business, Competitor } from "@/lib/store";
...
export default function CompetitorsPage() {
            ...
            <div className="p-8 h-screen overflow-y-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm">Compare your reputation against local rivals.</p>
                    </div>
                </div>
    ...
    const allScores = [{ name: "You", score: myCurrentScore }, ...competitors.map(c => ({ name: c.name, score: history[history.length - 1]?.[c.name] || 0 }))];
    ...
    return (
        <div className="p-8 h-screen overflow-y-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-white mb-1">Competitor Benchmarking</h1>
                    <p className="text-muted-foreground text-sm">Compare your reputation against local rivals over the last 6 months.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="glass-card p-5 rounded-2xl flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-white">#{myRank} <span className="text-sm font-normal text-muted-foreground">of {allScores.length}</span></p>
                    </div>
                ...
                <div className="glass-card p-5 rounded-2xl flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-lg font-bold text-white truncate max-w-[150px]">{allScores[0].name}</p>
                ...
                <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={history} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <YAxis domain={['dataMin - 0.2', 'dataMax + 0.2']} stroke="#94a3b8" tick={{ fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                            <Tooltip
                                contentStyle={{ backgroundColor: 'rgba(8, 11, 20, 0.95)', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }}
                                itemStyle={{ color: '#e2e8f0' }}
                            />
                            <Legend wrapperStyle={{ paddingTop: "20px" }} />
                            <Line type="monotone" dataKey="You" stroke={COLORS[0]} strokeWidth={4} activeDot={{ r: 8 }} />
                            {competitors.map((comp, idx) => (
                                <Line key={comp.id} type="monotone" dataKey={comp.name} stroke={COLORS[(idx + 1) % COLORS.length]} strokeWidth={2} strokeDasharray="5 5" />
                            ))}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
