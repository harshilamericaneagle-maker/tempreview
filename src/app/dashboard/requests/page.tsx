...
export default function RequestsPage() {
    ...
    const refresh = () => {
        ...
        if (!biz) return;
        setBusiness(biz);
        setCampaigns(getCampaignsByBusiness(biz.id).filter(c => !activeLocation || c.locationId === activeLocation.id));
    };
                    ...
                    <div className="glass-card rounded-2xl p-6 w-full max-w-3xl border border-border flex flex-col md:flex-row gap-8">
                        ...
                        <div className="flex-1 space-y-4">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-xl font-bold text-white flex items-center gap-2"><QrCode className="w-5 h-5 text-primary" /> QR Code Flyer</h3>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-muted-foreground mb-1">Headline Text</label>
                                <textarea rows={3} value={flyerText} onChange={e => setFlyerText(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-secondary/50 border border-border focus:border-primary focus:outline-none text-sm resize-none text-white" />
                                <p className="text-[10px] text-muted-foreground mt-1">This text will appear in large letters above the QR code.</p>
                            </div>
                            <div className="pt-4 flex flex-col gap-3">
                                <button onClick={() => window.print()} className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-primary text-white text-sm font-bold hover:bg-primary/90 transition-colors">
                                    <Printer className="w-4 h-4" />
                                    Print Flyer
                                </button>
                                <button onClick={() => setShowFlyerModal(false)} className="w-full text-center px-5 py-3 rounded-xl bg-secondary text-white text-sm font-bold hover:bg-secondary/80 transition-colors">
                                    Cancel & Close
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex-shrink-0 flex items-center justify-center bg-white/5 rounded-xl p-4 border border-border/50">
                            <div id="printable-flyer" className="w-[300px] h-[400px] bg-white rounded-2xl flex flex-col items-center justify-center p-8 text-center shadow-2xl relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-3 bg-primary"></div>
                                <h2 className="text-xl font-extrabold text-slate-800 mb-8 leading-tight">{flyerText}</h2>
                                ...
                                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">{business.name}</p>
                            </div>
                        </div>
                    </div>
                </div>
