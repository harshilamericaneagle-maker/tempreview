        ...
        {insights.length === 0 ? (
          <div className="glass-card rounded-2xl p-12 text-center">
            ...
            <p className="text-muted-foreground">
              Not enough review data yet to generate insights. Keep collecting reviews!
            </p>
          </div>
        ) : (
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {group.items.map((insight) => {
                      ...
                      return (
                        <div
                          key={insight.id}
                          className={`glass-card rounded-2xl p-6 transition-all ${done ? "opacity-50" : ""}`}
                        >
                              ...
                              <div className="flex items-start justify-between gap-4 mb-2">
                                <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${PRIORITY_STYLES[insight.priority]}`}
                                    >
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                      · Based on {insight.basedOnCount} review
                                      {insight.basedOnCount !== 1 ? "s" : ""}
                                    </span>
                                  </div>
                                </div>
                                <button
