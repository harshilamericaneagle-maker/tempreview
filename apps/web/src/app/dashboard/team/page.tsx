      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Track staff performance, tasks resolved, and positive guest mentions.
          </p>
        </div>
      </div>
        ...
        <div className="glass-card p-5 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-white">
              {stats.reduce((acc, s) => acc + s.tasksResolved, 0)}
            </p>
          </div>
        </div>
        <div className="glass-card p-5 rounded-2xl flex items-center gap-4">
      ...
      {stats.length === 0 ? (
        ...
        <div className="glass-card rounded-2xl overflow-hidden border border-border">
          ...
          <div className="divide-y divide-border/50">
            {stats.map((stat, i) => {
              const rankStyle =
                i < 3 ? topRankColors[i] : "text-muted-foreground border-transparent";
              return (
                <div
                  key={stat.user.id}
                ...
                >
                  <div className="col-span-1 flex justify-center">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center border font-bold text-sm ${rankStyle}`}
                    >
                      {i === 0 ? (
